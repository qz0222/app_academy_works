...
def craiglockhart_to_sighthill
  ...
  FROM
    routes start
  JOIN
    stops stop_start ON (start.stop_id = stop_start.id)
  JOIN
    routes b ON (start.company = b.company AND start.num = b.num)
  JOIN
    routes transfer ON (b.stop_id = transfer.stop_id)
  JOIN
    stops transfer_stop ON (transfer_stop.id = transfer.stop_id)
  JOIN
    routes transferb ON
      transferb.num = transfer.num AND
      transferb.company = transfer.company
  JOIN
    stops final_stop ON (final_stop.id = transferb.stop_id)
  WHERE
    stop_start.name = 'Craiglockhart' AND
    final_stop.name = 'Sighthill'
